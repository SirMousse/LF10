...
*/
// Lösung 1:
function derSprechenderHut($name)
{
    return $name . " kommt nach Gryffindor!";
}

echo derSprechenderHut("Hermine") . "<br>";

// Lösung 2:
function sprechenderHut(string $name): string
{
    return "{$name} kommt nach Gryffindor!";
}

echo sprechenderHut("Hermine") . PHP_EOL;
...
function pokemonRuf(string $pokemon): string
{
    return "{$pokemon}! {$pokemon}!";
}

echo pokemonRuf("Pikachu") . PHP_EOL;
...
function zombieBiss(int $leben): int
{
    return max(0, $leben - 10);
}

echo zombieBiss(50) . PHP_EOL;
